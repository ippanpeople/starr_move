...
if (username === null || username === "") {
    ...
    receivedUserList.forEach(user => {
      if(user["connection_status"] == "connected"){
        if(username != user["username"]){
          ...
          chatUsersList.push(user["username"])
          var option = document.createElement("option");
    ...
    if (resp_event == "init_resp_event") {
      ...
      for (let i = 0; i < receivedData.user_list.length; i++) {
        ...
        if(userDict.connection_status ==  "connected"){
          ...
          }else if(positionX <= 21 && positionY <=25 && positionX >= 5 && positionY >= 14){
            if(positionX <= 21 && positionY <=25 && positionX >= 5 && positionY >= 14){
            console.log("userdict")
  ...
  chatSendButton.addEventListener('click',() => {
    ...
    console.log(text)
    const chat_user_select = document.getElementById("chat-user-select").value
    if(chat_user_select == "") {
      chatUsers = []
    }else{
      chatUsers.push(chat_user_select)
    }
    console.log(chat_user_select)
      const positionData = {
        event: "chat_event",
        username: username,
        x: positionX,
